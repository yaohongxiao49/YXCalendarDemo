...
@implementation YXSeparationManager

#pragma mark - 单例
+ (instancetype)sharedManager {
    
    static YXSeparationManager *manager = nil;
...
+ (YXSolarModel *)solarFromInt:(long)g {
    ...
    YXSolarModel *solar = [[YXSolarModel alloc] init];
...
+ (YXSolarModel *)solarFromLunar:(YXLunarModel *)lunar {
    ...
    //闰月范围1900-2050，可自行增加，农历不在这个范围时计算公历会出错
    NSDictionary *leapDic = @{@"1900":@"8",
                              @"1903":@"5",
                              @"1906":@"4",
                              @"1909":@"2",
                              @"1911":@"6",
...
+ (YXLunarModel *)lunarFromSolar:(YXSolarModel *)solar {
    ...
    YXLunarModel *lunar = [[YXLunarModel alloc] init];
...
+ (NSString *)lunarYearFromSolarYear:(NSInteger)solarYear {
    ...
    //干指天干
    NSArray *ganArr = @[@"甲", @"乙", @"丙", @"丁", @"戊", @"己", @"庚", @"辛", @"壬", @"癸"];
    //支指地支
    NSArray *zhiArr = @[@"子", @"丑", @"寅", @"卯", @"辰", @"巳", @"午", @"未", @"申", @"酉", @"戌", @"亥"];
    //天干计算法：年减去3再除以10得到的余数对应的天干字就是干
    //地支计算法：年减去3再除以12得到的余数对应的地支字就是支
    NSInteger ganIndex = (solarYear - 3) %10;
    NSInteger zhiIndex = (solarYear - 3) %12 ;
    ganIndex = (ganIndex == 0) ? 10 - 1 : ganIndex - 1;
    zhiIndex = (zhiIndex == 0) ? 12 - 1 : zhiIndex - 1;
    return [NSString stringWithFormat:@"%@%@年", ganArr[ganIndex], zhiArr[zhiIndex]];
}

#pragma mark - 农历转字符串
+ (NSString *)stringFromLunar:(YXLunarModel *)lunar {
    
    NSArray *lunarMonthArr = [self getLunarMonthArr];
    NSArray *lunarDayArr = [self getLunarDayArr];
    return [NSString stringWithFormat:@"%d%@%@", lunar.lunarYear, lunarMonthArr[lunar.lunarMonth - 1], lunarDayArr[lunar.lunarDay - 1]];
}
